...
*/
 ...
 for (var index = 0; index < required_parameters.length; index++) {
  ...
  if(!(good_parameters.includes(required_parameters[index]))) {
   ...
   else if((ans['bad_values'].includes(required_parameters[index])) && (print_missing_parameters)) {
    console.log(txt['user']+required_parameters[index] +txt['end']+ " was passed in as a Value without an associated Parameter");
   }

  }

  else if(!(argv[required_parameters[index]] === undefined)){
   // Debug
   ...
   //ans['bad_values'].push(argv[required_parameters[index]]);
  }
 ...
 if(!required_parameter_check_passed) {
  ...
  // Debugging
  if(print_missing_parameters) {
   console.log("\n< "+txt['notify']+"Required"+txt['end']+" Parameters were "+txt['passed']+"met"+txt['end']+" >");
  }
  
 }

 //for(index = 0; index < ans['good_parameters'].length;index++) {
 // console.log(ans['good_parameters'][index] + ":" + ans['good_values'][ans['good_parameters'][index]]);
 //}
